...
### CSS Base (Compilado desde SCSS)
- Todas las variables CSS estructurales están precompiladas desde SCSS a `nui.css` (518KB)
- Incluye ~500+ variables: espaciado, tamaños, bordes, sombras, transiciones, etc.
- Generado usando mixins complejos de SCSS (tint, shade, colores complementarios)
- **No requiere `@import`** en proyectos consumidores
- **No necesita `stylePreprocessorOptions`** en angular.json

### Colores Dinámicos (CSS-in-JS)
- Solo 7 valores de color semánticos × 2 modos = **14 variables de color**
- Inyectadas dinámicamente via `ThemeService`
- Permite cambio de temas en tiempo de ejecución
- Sobrecarga mínima (~200 líneas de TypeScript vs 2000+ para CSS-in-JS completo)

## ¿Por qué un Enfoque Híbrido?

### ❌ Enfoque CSS-in-JS Puro (Abandonado)
**Problema:** Requeriría replicar 900+ líneas de lógica de mixins SCSS en TypeScript
- `generate-button-variables()` crea 8 variables por color por variante
- `generate-chip-variables()`, `generate-toast-variables()`, etc.
- Cálculos dinámicos de tint/shade
- Generación de colores complementarios (rotación de hue)
- Extracción de componentes RGB

**Tiempo estimado:** 7-10 horas con alto riesgo de errores

### ✅ Enfoque Híbrido (Actual)
**Solución:** Aprovechar la compilación SCSS existente + solo colores dinámicos
- CSS base precompilado una vez
- CSS-in-JS solo para colores de tema
...
#### Opción B: En styles.scss
```scss
@import 'nui/styles/nui';
```

**Nota:** Para la Opción B, necesitarás configurar `stylePreprocessorOptions` en `angular.json`:
```json
{
...
✅ **Presets listos** - 6 temas predefinidos para empezar  
