export type TimeSelectorMode =
  ...
  | 'HOUR_MINUTE_24'
  | 'DURATION';
export enum TimeSelectorModeEnum {
  HOUR_12 = 'HOUR_12',
  HOUR_24 = 'HOUR_24',
...
export type TimeSelectorSection =
  | 'hour'
  | 'minute'
  | 'period'
  | 'duration-hours'
...
export type TimeSelectorStrategy = 'now' | 'smart' | 'empty' | 'custom';
export enum TimeSelectorStrategyEnum {
  NOW = 'now',
  SMART = 'smart',
  EMPTY = 'empty',
  CUSTOM = 'custom',
...
/**
 * Tipo de valor que acepta el time-selector
 * Puede ser:
 ...
 */
export type TimeSelectorValue = TimeValue | DurationValue | Date | string | null;
 ...
 */
export interface ResolvedTimeSelectorConfig {
  hourStep: number;
  minuteStep: number;
  minTime: TimeValue | undefined;
  maxTime: TimeValue | undefined;
...
export const DEFAULT_CONFIG: ResolvedTimeSelectorConfig = {
  hourStep: 1,
  minuteStep: 1,
  minTime: undefined,
  maxTime: undefined,
