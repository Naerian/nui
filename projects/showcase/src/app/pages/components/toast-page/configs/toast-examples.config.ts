...
setTimeout(() => {
  ...
  },
  {
    ...
    title: 'components.toast.examples.native.title',
    description: 'components.toast.examples.native.description',
    note: {
      type: 'info',
      content: 'components.toast.examples.native.note',
    },
    anchor: 'native-notifications',
    examples: [
      {
        title: 'app.config.ts (Global configuration)',
        code: `import { NativeNotificationStrategy } from 'nui';

provideNUI({
  config: {
    toast: {
      // Native on mobile, NUI toast on desktop
      nativeNotifications: NativeNotificationStrategy.Mobile,
    }
  }
});`,
        language: 'typescript',
      ...
      {
        title: 'codeExamples.usage',
        code: `import { NativeNotificationStrategy, ToastService } from 'nui';
...
async onEnableNotifications() {
  ...
  if (result === 'granted') {
    ...
    this.toastService.warning('Permission denied. Enable it from the browser settings.');
  }
}

// 2. Normal usage — the service automatically decides based on the strategy
this.toastService.success('Operation completed successfully'); // Will use native or NUI toast based on the configured strategy

// 3. Override per individual toast
this.toastService.error('Critical error', {
  nativeStrategy: NativeNotificationStrategy.Always,
});

// 4. Read the current permission state reactively
readonly permission = this.toastService.nativePermission; // Signal<'granted'|'denied'|'default'|'unsupported'>`,
        language: 'typescript',
      },
    ],
  },
