# TimeSelector Component - Documentación Completa

## Resumen

El componente `TimeSelector` es un selector de tiempo altamente accesible y configurable que cumple con los estándares WCAG 2.1 AA. Permite a los usuarios seleccionar horas, minutos y período (AM/PM) usando tanto mouse como navegación por teclado completa.
...
```scss
// Variables de tamaño
--time-selector-width-sm: 280px;
--time-selector-width-md: 320px;
--time-selector-width-lg: 360px;
...
```html
<div class="sr-only" id="time-selector-help">
...
```
time-selector/
├── time-selector.component.ts      # Lógica principal y navegación
├── time-selector.component.html    # Template con ARIA
├── time-selector.component.scss    # Estilos con CSS Custom Properties
└── time-selector.component.spec.ts # Tests unitarios
```
...
officeHoursConfig: TimeSelectorConfig = {
  format: '24h',
...
appointmentConfig: TimeSelectorConfig = {
  format: '12h',
  hourStep: 1,
  minuteStep: 15,
...
```html
<nui-time-selector></nui-time-selector>
```
...
|-----------|------|---------|-------------|
| `mode` | `TimeSelectorMode` | `'HOUR_MINUTE_24'` | Modo del picker (ver tipos abajo) |
...
| `config` | `TimeSelectorConfig` | `{}` | Configuración avanzada (steps, min/max, presets, etc.) |
...
type TimeSelectorSection = 'hour' | 'minute' | 'second' | 'period';
...
import { TimeValue } from '@shared/components/time-selector/models/time-selector.model';
...
```typescript
import { TimeSelectorConfig, TimeValue } from '@shared/components/time-selector/models/time-selector.model';

export class ExampleComponent {
  // Configuración para horario laboral (24h)
  workHoursConfig: TimeSelectorConfig = {
    minTime: { hour: 8, minute: 0 },
    maxTime: { hour: 18, minute: 0 },
    minuteStep: 30
  };
...
<nui-time-selector defaultStrategy="empty"></nui-time-selector>

<!-- Hora actual -->
<nui-time-selector defaultStrategy="now"></nui-time-selector>
...
```html
<nui-time-selector size="xs"></nui-time-selector>
<nui-time-selector size="s"></nui-time-selector>
<nui-time-selector size="sm"></nui-time-selector>
<nui-time-selector size="md"></nui-time-selector>
<nui-time-selector size="lg"></nui-time-selector>
<nui-time-selector size="xl"></nui-time-selector>
```
...
```typescript
import { TimePreset, TimeValue } from '@shared/components/time-selector/models/time-selector.model';
...
```typescript
import { TimeSelectorConfig, DurationPreset } from '@shared/components/time-selector/models/time-selector.model';
...
export class ExampleComponent {
  ...
  durationConfig: TimeSelectorConfig = {
    duration: {
      maxHours: 12,
      showSeconds: false,
...
### Agenda/Calendario con TimeSelector
...
import { DurationValue } from '@shared/components/time-selector/models/time-selector.model';
...
### TimeSelector con Validación Personalizada
...
```html
<nui-time-selector formControlName="meetingTime"></nui-time-selector>
...
### Múltiples TimeSelectors Sincronizados
...
nui-time-selector {
  --time-selector-bg: white;
  --time-selector-text: #333;
  --time-selector-selected-bg: #0066cc;
  --time-selector-selected-text: white;
  --time-selector-hover-bg: #f0f0f0;
  --time-selector-disabled-text: #ccc;
}

// Personalizar tamaños
nui-time-selector {
  --time-selector-width: 200px;
  --time-selector-dropdown-width: 240px;
  --time-selector-item-height: 32px;
}

// Personalizar border
nui-time-selector {
  --time-selector-border-radius: 6px;
  --time-selector-border-color: #d0d0d0;
}
```
...
El TimeSelector utiliza **CSS Custom Properties** del sistema de temas NUI y se adapta automáticamente según el tema activo.

### Variantes

```html
<!-- Variante por defecto -->
<nui-time-selector variant="default"></nui-time-selector>

<!-- Variante compacta (menos padding, más condensado) -->
<nui-time-selector variant="compact"></nui-time-selector>
```
...
```html
<nui-time-selector mode="DURATION"></nui-time-selector>
```
...
- [Calendar Component](./calendar.md) - Integración perfecta con TimeSelector
